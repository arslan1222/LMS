...
const CourseDetails = () => {
  ...
  const  { allCourses, calculateRating, getToken, calculateChapterTime, calculateLectures, calculateCourseDuration, currency, backendUrl, userData } = useContext(AppContext);
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
          ...
          <p className='text-primary2'>({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'Ratings' : 'Rating'})</p>
          <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'Students' : 'Student'}</p>
        </div>

        <p className='text-sm'>Course by <span className='text-primary2 underline'>{courseData.educator.name}</span></p>
        <div className='pt-8 text-gray-800'>
          <h2 className='text-xl font-semibold text-primary2'>Course Structure</h2>

          <div className='pt-5'>
            {
              courseData.courseContent.map((chapter, index)=>(
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                  <div className='flex itmes-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=> toggleSection(index)}>
                  ...
                  <div className={`overflow-hidden transition-all duration-600 ${openSections[index] ? 'max-h-96' : 'max-h-0'}`}>
                          ...
                          <li key={index} className='flex items-start gap-2 py-1'>
                            ...
                            <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                              ...
                              <div className='flex gap-2 '>
                                  ...
                                  })} className='text-primary2 underline cursor-pointer'>Preview</p>}
        ...
        <div className='py-20 text-sm md:text-default'>
          <h3 className='text-xl font-semibold text-primary2'>Course Description</h3>
      ...
      {/* Right Section */}
      <div className='max-w-course-card shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-shite min-w-[300px] sm:min-w[420px] z-10'>
        ...
        <div className='p-5'>
          ...
          <div className='flex gap-3 itmes-center pt-2'>
            <p className='text-primary2 md:text-4xl text-2xl font-semibold'>{currency} {(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
